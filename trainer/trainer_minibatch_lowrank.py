...
def train(config):
    ...
    L = torch.nn.Parameter(torch.rand(1024).to(device).uniform_(0, 0.1))
    ...
    criterion = MahalalobisL2(epsilon=config.training.epsilon, reg=config.training.reg, use_cosine=config.training.use_cosine)
    ...
    #     criterion = OTLoss(epsilon=config.training.epsilon)
    ...
    for epoch in range(ep, config.training.epochs + 1):
        ...
        for batch_id, batch_data in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            shuffle_cap = captions[:int(len(captions)*noise_p)]
            random.shuffle(shuffle_cap)
            captions[:int(len(captions)*noise_p)] = shuffle_cap
            # move data to GPU
...
def validate(data_loader, model, device, writer, epoch, L, use_ot=False, use_cosine=True):
    ...
    with torch.no_grad():
        ...
        r1, r5, r10, r50, medr, meanr, crossentropy_t2a  = t2a_ot_kernel(audio_embs, cap_embs, M,use_ot, use_cosine)
        r_sum_t2a = r1 +r5 + r10
        t2a_metrics['r1'] += r1
        t2a_metrics['r5'] += r5
        t2a_metrics['r10'] += r10
        ...
        r1_a, r5_a, r10_a, r50_a, medr_a, meanr_a, crossentropy_a2t = a2t_ot_kernel(audio_embs, cap_embs, M,use_ot, use_cosine)
...
def validate_a2t(data_loader, model, device, use_ot, use_cosine, L):
    ...
    with torch.no_grad():
        ...
        r1_a, r5_a, r10_a, r50_a, medr_a, meanr_a,_ = a2t_ot_kernel(audio_embs, cap_embs, M,use_ot, use_cosine)
        a2t_metrics['r1'] += r1_a
        a2t_metrics['r5'] += r5_a
        a2t_metrics['r10'] += r10_a
        a2t_metrics['median'] += medr_a
...
def validate_t2a(data_loader, model, device, use_ot, use_cosine, L):
    ...
    with torch.no_grad():
        ...
        r1, r5, r10, r50, medr, meanr,_ = t2a_ot_kernel(audio_embs, cap_embs, M,use_ot, use_cosine)
        t2a_metrics['r1'] += r1
        t2a_metrics['r5'] += r5
        t2a_metrics['r10'] += r10
        t2a_metrics['median'] += medr
